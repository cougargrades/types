...
export function generateKeywords(firstName: string, lastName: string) {
    ...
    let permutations = k < 5 ? generatePermutations(fullName) : generateConservativePermutations(firstName, lastName)
    let result: string[] = []
...
export function generatePermutations(fullName: string) {
    ...
    for(let i = 0; i < names.length; i++) {
        permutations = permutations.concat(Array.from(itertools.permutations(names, i)));
    }
...
export function generateConservativePermutations(firstName: string, lastName: string) {
    return [
        firstName,
        lastName,
        `${firstName} ${lastName}`,
